...
> _Uma estrutura de dados (ED), em ciência da computação, é uma coleção tanto de valores (e seus relacionamentos) quanto de operações (sobre os valores e estruturas decorrentes). [Wikipédia](https://pt.wikipedia.org/wiki/Estrutura_de_dados)_

Para o nosso cenário atual, vamos considerar uma estrutura de dados como uma coleção ou conjunto de valores.
...
Como fazermos para representar estas diferentes listas em nosso programa? Justamente através das `estrutura de dados`.

Nossa `estrutura de dados` podem ser vista de maneira mais simplificada, como `variáveis` ou `objetos` que são capazes de armazenar `um` ou `mais` valores ao mesmo tempo. Essa estrutura de dados também é conhecida como `array`.

No Python, as principais estruturas são:

* `Listas` (listas mutáveis/alteráveis)
* `Sets` (conjuntos parcialmente mutáveis que podem ser alterados)
* `Dicionários` (conjunto formado por pares de `chave` e `valor`)
* `Tuplas`: (listas imutáveis que não podem ser alteradas)

Cada `estruturas de dados` resolve um tipo de problema diferente e podem ser úteis em diversas situações.
...
3. Caso contrário, execute o outro trecho de código.
...
# 0
```
No exemplo acima, apenas o primeira número será mostrado, pois o `break` logo após interrompe o fluxo do laço.
...
Indentação nada mais é do que um `recuo` ou `avanço` de `espaçamento` na formatação do código. A `indentação` é utilizada para `ressaltar` ou até mesmo `definir` a estrutura do código, aumentando assim a legibilidade do mesmo.
...
O código acima, apesar de ter a `sintaxe` correta, possui um problema de `indentação`. Como o Python não utiliza `delimitadores` para seus blocos de código, como `{}` utilizadas na linguagem `C`, o interpretador Python espera que todo o código pertencente ao bloco `for`, esteja devidamente `alinhado` com um `tab` ou `4 espaços` para frente, como no exemplo a seguir:
...
Funções em linguagem de programação podem ser entendidas como um `recurso` ou `objeto` separado do algoritmo principal que contém um bloco de código que pode ser executado várias e várias vezes da mesma forma.
...
* Uma função pode ser executada passando-se um determinado `0 ou n` número(s) de argumentos de entrada.
...
O código acima `centralizou`  a lógica para imprimir o resultado dentro da função `somar()`, simplificando o fluxo principal do programa. Isso também ajuda a melhorar a manutenabilidade, já que só precisamos mexer em um único lugar do nosso programa.
...
O código acima demonstra o método `enviar_email()` que recebe os parâmetros `email` e `texto`. Perceba que não exista a palavra `return` ao final. Este método apenas irá enviar o e-mail e nenhum resultado será retornar para quem chamou este método. Perceba também a utilização da palavra `pass`, ela é utilizada quando ainda não temos nenhuma linha de código escrita dentro do `método`, sendo muito utilizada quando queremos `postegar` a definição do `método` para outro momento.
...
As bibliotecas podem ser instaladas diretamente no diretório do interpretador Pythonm através de utilitários de linha de comando como o `pip`, e importadas para dentro do seu projeto, sempre que necessário. Assim o código do seu programa fica mais enxuto.
...
Outro exemplo é utilizando a biblioteca `requests`, necessário para se realizar chamadas `http` a algum site:
...
O código acima define e abstrai um `gato` do mundo real para o mundo virtual da programação. Perceba que é possível espelhar com a programação qualquer `características` e `ação` do `gato`.
...
O código acima `instância` dois gatos direrentes `meu_gato_favorito` e `gato_do_vizinho` que possuem `características (atributos)` diferentes, sendo o primeiro da cor `preta` e o segundo da cor `branco`. Lembra que comentamos acima que a `classe` serve como um `molde`? Pois é exatamente o que está acontecendo aqui. Sempre que precisarmos inserir um `gato` em nosso programa, podemos utilizar a classe `Gato`. Assim, podemos representar vários `gatos` no nosso mundo virtual da proramação, cada um totalmente independente com as suas próprias `características (atributos)`.
...
Para capturar o valor de uma propriedade basta indicar qual o objeto que contém a `instância` da classe segido de `. (ponto)` e o nome da propriedade, sem utilizar os símbolos `()`:
...
Para executar um método em uma classe basta indicar qual o objeto que contém a `instância` da classe segido de `. (ponto)` e o nome da `função/método`, seguido dos símbolos `() parênteses`. Caso a `função/método` receba algum parâmetro de entrada, estes devem ser especificados dentro dos `() parênteses`:
...
Perceba que no código acima a classe `Gato` não possui uma definição para o método `comer`. No entanto, como a classe `Gato` herda a classe `Animal`, automagicamente a classe `Gato` passa a ter acesso a todos os comportamentos da classe pai `Animal`.
